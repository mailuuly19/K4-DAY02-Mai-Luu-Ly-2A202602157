...
- Mã SHA-256 của gói YOLO của bạn: `b8376f329fcee49ac5adaae4beee046cb104266acb5f133a53479b8895e8a358`
- Mã SHA-256 của gói CVAT gốc của bạn: `00b911bc489c4b2707161b6bbd6ee461b25189de8bd7f6acd2812b52d66403ca`
...
| --- | --- | --- | --- |
| Gói YOLO đầu tiên có thứ tự `0 car, 1 truck, 2 van, 3 bus` | lớp/schema | Ô 3a dừng và báo schema export không khớp `0 car, 1 truck, 2 bus, 3 van`; kiểm tra `data.yaml` xác nhận `bus` và `van` bị đảo | Sửa cấu hình lớp từ gốc trong CVAT, kiểm tra lại tên lớp của các hộp rồi xuất lại cả YOLO và CVAT từ cùng trạng thái. Gói mới có đúng thứ tự `0 car, 1 truck, 2 bus, 3 van` |

- Số hộp `needs_review` trước và sau khi kiểm: notebook không lưu số lượng trước khi kiểm nên không có căn cứ để ghi một số giả; bản xuất hiện tại còn `2` hộp `needs_review` trên tổng số `67` hộp.
- Một quyết định chưa đủ bằng chứng và cách bạn xin hỗ trợ: hộp `car` thứ 17 trong `drive_033` có tọa độ khoảng `(274.05, 70.75, 289.89, 91.87)`, được đánh dấu `visibility=unclear`, `boundary=inside`, `review_state=needs_review` vì vật thể rất nhỏ. Tôi sẽ phóng ảnh lên 100%, nêu dấu hiệu quan sát được và nhờ Lab Coach xác nhận có đủ bằng chứng để giữ lớp `car` hay nên loại khỏi phạm vi thay vì tự đoán.
...
- Dự đoán đó gợi ý cần kiểm lại quy tắc hoặc dữ liệu nào? Cần kiểm tra lại tính đầy đủ và nhất quán của nhãn, đặc biệt là các lớp ít mẫu như `truck`, `bus`, `van`, đồng thời kiểm tra số hộp nhỏ hoặc bị che và sự chênh lệch giữa 67 hộp của tôi với 50 hộp tham chiếu.
...
- Số hộp phía tôi không ghép được: `27`
...
- Vì sao mức đồng thuận cao không chứng minh mọi nhãn đều đúng? Hai nguồn có thể cùng bỏ sót một vật thể, cùng áp dụng sai quy tắc hoặc cùng vẽ hộp sai nhưng vẫn đồng thuận với nhau. Chỉ số đồng thuận cũng chỉ được tính trên các hộp ghép được và không tự giải thích 27 hộp của tôi cùng 10 hộp tham chiếu chưa ghép được.
...
Minh chứng mạnh nhất là hai gói xuất YOLO và CVAT có cùng 67 hộp, cùng bốn ảnh và đạt kiểm tra nhất quán chéo với 67 hộp được ghép, IoU nhỏ nhất khoảng `0.999954`; điều này chứng minh hai định dạng được xuất từ cùng trạng thái CVAT. Câu hỏi còn lại cho Lab Coach là: với 27 hộp của tôi không ghép được và tổng số 67 hộp cao hơn 50 hộp tham chiếu, những vật thể nhỏ hoặc mờ nào vẫn đủ bằng chứng để giữ lại theo quy tắc phạm vi của bài?
